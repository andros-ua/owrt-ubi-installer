...
#
# The script:
#   1. Downloads and verifies an OpenWrt Image Builder (IB) snapshot via PGP + sha256
...
prepare_openwrt_ib() {
	...
	# Always re-fetch the checksum manifest — it changes with every snapshot.
...
#   - Renames the SPI-NAND compatible string so U-Boot ignores it during
#     the installation reboot (prevents double-init races)
#   - Adds an "old_ubi" alias for the full flash region starting at
#     0x580000 so the installer script can locate and erase it by label
# ---------------------------------------------------------------------------
allow_mtd_write() {
...
bundle_initrd() {
	...
	# resolve the correct package versions from the snapshot feed.
	...
	case "$imgtype" in
		...
		installer)
			...
			allow_mtd_write
			# The installer uses an external, non-static FIT layout.
			EXTERNAL=
			STATIC=
